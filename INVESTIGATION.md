...
- `keyspark` tmux session is up (4 windows: agent, streaming, backend, frontend).
- `keyspark-kafka` container: healthy, up 2 days.
...
- Fix: `keyspark/api.py`. In `_run_batch`, on any exception during `compute_all`, update `batch_state` with `last_status=failed` and a fresh `last_run`, then call `os._exit(1)`. The outer bash restart loop in startup-tmux.sh respawns the API with a fresh JVM, and lifespan rebuilds the Spark session cleanly. Added `import os`.
...
- Fix:
  ...
  - `keyspark/streaming_job.py`: dropped `WATERMARK` from 30 s to 5 s. This brings the first-emit lag from 90 s down to 65 s, which matters for criterion 4 (wake-to-green) and Attack 1 (cold-start-to-green). The 1-minute window is preserved so per-minute frontend labels stay accurate.
...
- Fix: `startup-tmux.sh`. After Kafka becomes reachable, the script now:
  1. `docker exec keyspark-kafka /opt/kafka/bin/kafka-topics.sh --create --if-not-exists --topic events.raw ...` (idempotent).
...
KeySpark started in detached tmux session 'keyspark'.
...
```
$ tmux capture-pane -t keyspark:0 -p -S -2000 | head
2026-05-27 22:46:41,352 WARNING keyspark.agent: macOS Accessibility not granted to this process - pynput may not capture keyboard events ...
2026-05-27 22:46:41,352 INFO keyspark.agent: agent listening (sink=kafka user=user-001) - waiting for input events
...
- Cold start at 2026-05-27T22:40:51 MSK: `tmux kill-session`, `pkill -f keyspark`, `docker compose down`, `find . -name __pycache__ -exec rm -rf`, then `./startup-tmux.sh`.
...
- 2026-05-27T22:40:51 MSK: full reset (`tmux kill-session`, `pkill keyspark`, `docker compose down`, `find . -name __pycache__ -exec rm -rf`).
...
- I re-walked every "proof after" entry by issuing the same observation right now (2026-05-27T20:09Z):
  ...
  - Bug 6: `keyspark/agent.py` lines 60-65 import AXIsProcessTrusted; lines 312-314 call it; the warning text matches what the tmux pane shows at 22:46:41 / 23:04:56.
...
- **My cold-start adversarial attack runs from a non-trusted shell**, so the agent comes up without Accessibility (Bug 6 fires its warning correctly but the agent cannot capture events). The code is correct; the user should rerun `tmux kill-session -t keyspark && ./startup-tmux.sh` from their iTerm2/Terminal.app to verify criterion 1's dot-green half in the real environment. After their re-run, the agent inherits the user's terminal's Accessibility and pynput captures events normally.

## Session log
- 2026-05-27T12:29Z: read README, startup.md, all four keyspark modules, frontend dot logic, docker-compose, startup-tmux.sh.
...
- All bug fixes are in the worktree (no commits made per the constraint). Files touched: `keyspark/agent.py`, `keyspark/api.py`, `keyspark/streaming_job.py`, `frontend/src/components/Header.jsx`, `startup-tmux.sh`. INVESTIGATION.md was created in the repo root.
